...
import { baseURL } from "./../Routes/urls";
...
function FetchProvider({children}) {
    ...
    const userDetailRef = useRef(getUserDetails);
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            ...
            try {
                let details = await userDetailRef.current(token);
                if (!triggered) {
                    setToken(token);
                    setUser({...details});
                    setLoggedIn(() => true);
    ...
    async function postWithToken(data, url, token, signal=null) {
